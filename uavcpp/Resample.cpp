...
*/
int Resample(const char *InFile, const char *OutFile, float fResX, float fResY,
             int iResampleAlg, int *pBandIndex, int pnBandCount) {
  GDALAllRegister();
  ...
  GDALDataset *pDDst =
      pDriver->Create(OutFile, newWidth, newHeight, iBandSize, dataType, NULL);
  ...
  hTransformArg = GDALCreateGenImgProjTransformer2((GDALDatasetH)pDSrc,
                                                   (GDALDatasetH)pDDst, NULL);
  ...
  papszWarpOptions =
      CSLSetNameValue(papszWarpOptions, "NUM_THREADS", "ALL_CPUS");
...
int main() {
  Resample("dem.tif", "result.tif", 60, 60, 8, NULL, 0);
  return 0;
}